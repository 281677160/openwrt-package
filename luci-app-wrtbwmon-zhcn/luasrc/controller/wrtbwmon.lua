...
function index()
    entry({"admin", "status", "usage"}, alias("admin", "status", "usage", "details"), _("流量统计"), 60)
    entry({"admin", "status", "usage", "details"}, template("wrtbwmon"), _("详情"), 10).leaf=true
    entry({"admin", "status", "usage", "config"}, cbi("wrtbwmon/config"), _("配置"), 20).leaf=true
    entry({"admin", "status", "usage", "custom"}, form("wrtbwmon/custom"), _("自定义"), 30).leaf=true
